# BIG todo: I have code for d-dim cube & boundary, interval boundary, and maybe more? 
...
def in_interval(xi: float, intervals) -> int:
    ...
    return ValueError(
        f'[in_interval] Invalid value to determine '
        f'which boundary to project to.'
    )
...
class HyperrectangleBoundary():
    ...
    def random_integration_points(self, key, N=50):
        ...
        for i in range(0, N):
            ...
            location = random.uniform(key_1, (), minval=0., maxval=length)
...
class HypercubeInitial():
    ...
    def __init__(self, dim):
        if not isinstance(dim, int):
            raise TypeError(
                f'[Constructor HypercubeInitial:] dim '
                f'must be integer'
            )
